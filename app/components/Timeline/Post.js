...
const Post = ({ post, onVote, onToggleComments, showComments, onComment, comments }) => {
  ...
  const { Content, publishedAt } = attributes;
  ...
  return (
    <div className="post mb-4 p-4 bg-white shadow-md rounded">
      <div className="post-header flex items-center">
        <img src="/path-to-default-user-image.png" alt="User" className="user-image w-10 h-10 rounded-full" />
        <div className="post-info ml-3">
          <p className="username font-bold">User Name</p>
        ...
        {post.media && (
          <div className="media-container mt-2">
            {post.media.type === 'video' ? (
              ...
              <img src={post.media.url} alt="Media" className="w-full mt-2" />
            )}
          </div>
        )}
      </div>
